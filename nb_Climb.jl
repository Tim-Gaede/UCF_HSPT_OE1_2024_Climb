...
begin
	...
	urlExt   = "Bees"
	probName = "Truffula Bees"
	origName = "Truffula Bees"
	
	filenameBase = "bees"
	fileIDRange = 1:16 #nothing #1:12 # ← nothing
	fileIDPad = 3
...
function parseDataset(raw)
	numNums = parse(Int, first(raw))

	nums = [parse(Int, subStr)     for subStr in split(raw[2])]
	
	if numNums != length(nums)
		msg = "Expected $numNums but got $(length(nums))."
		throw(ArgumentError(msg))
	end
...
# ╔═╡ 0b83d08a-bfcf-4582-a248-c527b01deffa
sqrs = [n^2   for n in 1:isqrt(10^5)]

# ╔═╡ 83441624-7350-403f-9aa9-1698325dd876


# ╔═╡ cdae0197-6952-4125-a751-274ffa4230a8
#=

	Timothy R. Gaede

	Put all squares into one group
	[1, 9, 49, 121, 144, 400]

	Then put all equal semi-squares into their own groups
	[8,8,8,8,8], [18, 18], [50, 50, 50], [200, 200, 200, 200]

	Then pair up nums that sum to a square
	[2,7], [7,9]..
...
function sln(favs)
	...
	nBeesWithFav = zeros(Int, 10^4)
	
	sqrs = [n^2   for n in 1:100]	
	isSquare = falses(10_000)
	for sqr in sqrs;     isSquare[sqr] = true;    end


	semiSquares = [sqrs[i] ÷ 2    for i in 2 : 2 : lastindex(sqrs)]
	isSemiSquare = falses(10_000)
	for semiSqr in semiSquares;     isSemiSquare[semiSqr] = true;    end
	return isSemiSquare
	
	#beeGrouped = falses(eachindex(favs))


	nSqrs = 0
	for i in eachindex(favs)
		 
		if isSquare[favs[i]]
			nSqrs +=1
			#beeGrouped[i] = true
		else
			nBeesWithFav[favs[i]] += 1
		end		 
	end
	...
	for i in eachindex(favs)
		if !isSquare[i]
			partnerFound = false
			s = 2
			while !partnerFound  &&  s ≤ 100

				partnerFav = sqrs[s] - favs[i]

				if partnerFav > 1  && nBeesWithFav[partnerFav] >= 1
					partnerFound == true
				else
					s+=1
				end
...
urlExnOnePerLine = "https://raw.githubusercontent.com/" * 
                   "Tim-Gaede/UCF_HSPT_OE1_2024_Bees/main/bees.out"
...
# ╔═╡ 67067fd6-94fe-4161-9a99-fe66d610df48
dsets = [parseDataset(part)   for part in inpParts]
...
# ╔═╡ 12a14215-85b3-48c5-8ea2-2538df93d130
sln(dsets[1])

# ╔═╡ fc166afe-4baa-4522-a59c-fbcf9705df79
dsets

# ╔═╡ d2c312f4-9570-428e-ba38-763b8041f56d
sln(dsets[3])

# ╔═╡ f367c579-68a9-4544-bc7a-07ab386ffc1e
slns = [sln(dsets[d])    for d in eachindex(dsets)]
